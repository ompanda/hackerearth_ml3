...
# #
# print("sampling")
# train = train.sample(frac=0.00001, replace=False)  # train.sample(1e6)
...
X_train, X_valid, Y_train, Y_valid = train_test_split(train, train.click, test_size = 0.1, random_state=2017)
...
exported_pipeline.fit(X_train[cols_to_use], Y_train)

print("Predicting on actual test data")

y_pred_final = exported_pipeline.predict(test[cols_to_use])

print ("AUC final ")

y_pred_train_stacked= exported_pipeline.predict(X_valid[cols_to_use])
...
output.to_csv(base_output_path+'stacking-v3-auc-{}.csv'.format(roc_stacked_pred), index=False)
