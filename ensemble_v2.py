...
X_train, X_valid, Y_train, Y_valid = train_test_split(train[cols_to_use], train.click, test_size = 0.1, random_state=2017)
...
dtest_test = xgb.DMatrix(test[cols_to_use])
...
print("averaing xgboost and stacked model score")
y__train_pred=average_precision_score(y_pred_train_stacked * 0.2855 + y__pred_train_xgb * 0.7145)

score= roc_auc_score(y_true = Y_valid, y_score=y__train_pred)

print("averaing xgboost and stacked model score is - {} ".format(score))
...
print ("saving xgb predictions")
output_xgb = pd.DataFrame({'ID': test['ID'], 'click': y_pred})
output_xgb.to_csv(base_output_path+'ensemble-v2-xgb-auc-{}.csv'.format(roc_xgb_pred), index=False)

print ("saving stacked predictions")
output_stacked = pd.DataFrame({'ID': test['ID'], 'click': results})
output_stacked.to_csv(base_output_path+'ensemble-v2-stacked-auc-{}.csv'.format(roc_stacked_pred), index=False)
...
output.to_csv(base_output_path+'ensemble-v2-auc-{}.csv'.format(score), index=False)
