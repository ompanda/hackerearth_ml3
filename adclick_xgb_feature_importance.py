...
# print("sampling")
# train = train.sample(frac=0.000001, replace=False)  # train.sample(1e6)
...
# # joining all files
agg_df = [site_offer_count, site_cat_count, site_mcht_count, site_ad_count, site_device_count, device_id_count,
          device_offer_count, merchant_device_count,
          category_ad_count, browser_ad_count, country_ad_count, country_offer_count, country_site_count,
          country_browser_count, category_id_count, tminute_id_count,
          tweek_id_count, thour_id_count, device_browser_count]
agg_df_test = [site_offer_count_test, site_cat_count_test, site_mcht_count_test, site_ad_count_test,
               site_device_count_test, device_id_count_test,
               device_offer_count_test, merchant_device_count_test, category_ad_count_test, browser_ad_count_test,
               country_ad_count_test, country_offer_count_test,
               country_site_count_test, country_browser_count_test, category_id_count_test, tminute_id_count_test,
               tweek_id_count_test, thour_id_count_test, device_browser_count_test]
...
# train model
model =XGBRegressor(max_depth=20,  n_estimators=1300, silent=True, objective='binary:logistic',
                    subsample=.95, colsample_bytree=0.4, base_score=y_mean, seed=seed) #xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round=1200)
...
# y_pred = model.predict(dtest_test)
y_pred = model.predict(stest)

print("print feature importance using xgboost inbuilt method")
# plot feature importance
plot_importance(model)
pyplot.show()

# print("print feature importance using custom method")
# # feature importance
...
print("calculating threashold using feature importance")
...
output.to_csv(base_output_path + 'adclick_xgb-feature-importance-v1-auc-{}.csv'.format(roc_xgb_pred), index=False)
...
#     # train model
#     selection_model = XGBRegressor(max_depth=20,  n_estimators=1300, silent=True, objective='binary:logistic',
#                     subsample=.95, colsample_bytree=0.4, base_score=y_mean, seed=seed)
...
#     print("Thresh=%.3f, n=%d, roc_auc_score: %.2f%%" % (thresh, select_X_train.shape[1], accuracy))
print("done")
