...
submission = base_output_path+'ftrl_v1_{}.csv'  # path of to be outputted submission file
...
for e in range(epoch):
    ...
    print("started training for epoch {}".format(e))

    for t,  ad_id, x, y in data(train, D):  # data is a generator
...
for t, ad_id, x, y in data(train, D):
    p = learner.predict(x)
    arr_y_train_pred.append(p)
    arr_y_train.append(y)

#y_train_pred= pd.DataFrame(columns="click")

score=roc_auc_score(y_true =np.array(arr_y_train) , y_score= np.array(arr_y_train_pred) )

print ("score is - {}".format(score))
print("submission")

with open(submission.format(score), 'w') as outfile:
    outfile.write('ID,click\n')
    for t,  ad_id, x, y in data(test, D):
        p = learner.predict(x)
        outfile.write('%s,%s\n' % (ad_id, str(p)))
        # if t % 1000000 == 0:
        #     print("Processed : ", t, datetime.now())
